...
use super::{DnsAdditional, DnsAnswer, DnsAuthority, DnsHeader, DnsQType, DnsQr, DnsQuestion, DnsResourceRecord, DnsResponseCode, DnsWireFormat, EdnsOptionCode, RData};
use crate::{EDNS_UDP_SIZE, EDNS_VERSION};
    ...
    pub fn create_response(&self) -> Self {
        ...
        if let Some(client_cookie) = self.get_client_cookie() {
            response.additional.push(
                DnsResourceRecord::new_opt_with_cookie(
                    EDNS_UDP_SIZE,
                    0,
                    EDNS_VERSION,
                    false,
