...
impl DnsWireFormat for DnsResourceRecord {
    ...
    fn from_wire(reader: &mut BitReader<Cursor<&[u8]>, BigEndian>) -> Result<Self, std::io::Error> {
        ...
        if let DnsQType::OPT = qtype {
            if let RData::OPT { ref mut udp_payload_size, ref mut extended_rcode, ref mut version, ref mut dnssec_ok, .. } = rdata {
...
impl DnsResourceRecord {
    pub fn new_opt_with_cookie(udp_payload_size: u16, extended_rcode: u8, version: u8, dnssec_ok: bool, client_cookie: &[u8]) -> Self {
        let mut options = Vec::new();
