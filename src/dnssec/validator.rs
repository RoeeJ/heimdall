...
use crate::dns::enums::{DNSResourceType, DNSResourceClass};
use super::{
    DnsSecAlgorithm, DigestType, DnsSecError, errors::Result,
    TrustAnchorStore, ValidationResult, calculate_key_tag,
    DenialOfExistenceValidator
};
...
impl DnsSecValidator {
    ...
    pub async fn validate(&self, packet: &DNSPacket) -> ValidationResult {
        debug!("Starting DNSSEC validation for packet ID {}", packet.header.id);
        
        // Check if response has DNSSEC records
        let has_rrsig = packet.answers.iter().any(|rr| rr.rtype == DNSResourceType::RRSIG)
            || packet.authorities.iter().any(|rr| rr.rtype == DNSResourceType::RRSIG);
    ...
    pub async fn validate_with_denial(
        ...
        debug!("Starting DNSSEC validation with denial check for {} {:?}", qname, qtype);
    ...
    async fn validate_rrsets(&self, packet: &DNSPacket) -> Result<()> {
        ...
        let mut rrsets: HashMap<(String, DNSResourceType, DNSResourceClass), Vec<&DNSResource>> = HashMap::new();
        ...
        for ((name, rtype, rclass), records) in rrsets {
            self.validate_rrset(&name, rtype, rclass, &records, packet).await?;
        }
    ...
    fn parse_rrsig(&self, rrsig: &DNSResource) -> Result<RrsigData> {
        ...
        let original_ttl = u32::from_be_bytes([
            rrsig.rdata[4], rrsig.rdata[5], rrsig.rdata[6], rrsig.rdata[7]
        ]);
        let sig_expiration = u32::from_be_bytes([
            rrsig.rdata[8], rrsig.rdata[9], rrsig.rdata[10], rrsig.rdata[11]
        ]);
        let sig_inception = u32::from_be_bytes([
            rrsig.rdata[12], rrsig.rdata[13], rrsig.rdata[14], rrsig.rdata[15]
        ]);
            ...
            .chain(packet.resources.iter())
        {
            ...
            if record.rtype == DNSResourceType::DNSKEY && record_name == rrsig.signer_name {
                ...
                if dnskey.key_tag == rrsig.key_tag && dnskey.algorithm == rrsig.algorithm {
                    ...
                    self.validate_dnskey(&dnskey, &rrsig.signer_name, packet).await?;
                    return Ok(dnskey);
                }
            }
        }
    ...
    fn parse_ds(&self, ds: &DNSResource) -> Result<DsData> {
        if ds.rdata.len() < 4 {
            return Err(DnsSecError::ValidationError("Invalid DS record".to_string()));
        }
    ...
    fn verify_signature(
        ...
        let verify_alg = algorithm.ring_algorithm()
            .ok_or(DnsSecError::UnsupportedAlgorithm(rrsig.algorithm))?;
