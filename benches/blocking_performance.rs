...
use heimdall::blocking::{BlockingMode, DnsBlocker};
...
fn bench_original_blocker(c: &mut Criterion) {
    ...
    group.bench_function("lookup", |b| {
        b.iter(|| {
            for domain in &test_domains {
                std::hint::black_box(blocker.is_blocked(domain));
            }
        });
    });
    ...
    group.bench_function("single_lookup", |b| {
        b.iter(|| {
            std::hint::black_box(blocker.is_blocked("test123.example.com"));
        });
    });
