use heimdall::{
    ...
    server::{run_udp_server, run_tcp_server},
};
...
async fn start_test_server(
    tcp: bool,
) -> (SocketAddr, broadcast::Sender<()>, tokio::task::JoinHandle<()>) {
    ...
    let resolver = Arc::new(DnsResolver::new(config.clone(), Some(metrics.clone())).await.unwrap());
...
async fn test_malformed_packet_handling() {
    ...
    assert!(result.is_ok(), "Server should still respond after malformed packet");
...
async fn test_rate_limiting() {
    ...
    assert!(responses <= 15, "Rate limiting should drop some queries (got {})", responses);
...
async fn test_max_concurrent_queries_limit() {
    ...
    assert!(successful >= 2, "At least concurrent limit queries should succeed");
