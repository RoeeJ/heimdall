use heimdall::blocking::arena::{SharedArena, StringArena};
use heimdall::blocking::builder::BlocklistBuilder;
use heimdall::blocking::lookup::{count_labels, DomainLabels, DomainNormalizer, extract_registrable_part};
...
"#;
    ...
    assert_eq!(psl.get_registrable_domain("www.example.com"), Some("example.com".to_string()));
    assert_eq!(psl.get_registrable_domain("test.example.co.uk"), Some("example.co.uk".to_string()));
    assert_eq!(psl.get_registrable_domain("something.random.uk"), Some("something.random.uk".to_string()));
    assert_eq!(psl.get_registrable_domain("metro.tokyo.jp"), Some("metro.tokyo.jp".to_string()));
}
...
fn test_psl_deduplication() {
    ...
    // Build and verify
    let (trie, arena, count) = builder.build().unwrap();
...
fn test_registrable_extraction() {
    ...
    assert_eq!(
        extract_registrable_part(b"com", 1),
        None
    );
}
