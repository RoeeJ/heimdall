...
use heimdall::config_reload::{ConfigReloader, ConfigChange, handle_config_changes};
...
async fn test_reload_from_env() {
    ...
    assert_eq!(change.new_config.bind_addr, "127.0.0.1:5353".parse::<SocketAddr>().unwrap());
...
"#).unwrap();
    ...
    let initial_config = DnsConfig::default();
    let mut reloader = ConfigReloader::new(initial_config, Some(temp_file.path().to_string_lossy().to_string()));
    let mut change_rx = reloader.take_change_receiver().unwrap();
    ...
    assert_eq!(change.new_config.bind_addr, "127.0.0.1:6363".parse::<SocketAddr>().unwrap());
    assert_eq!(change.new_config.upstream_servers.len(), 2);
...
async fn test_config_change_notification() {
    ...
    assert_ne!(change.old_config.enable_caching, change.new_config.enable_caching);
    assert_ne!(change.old_config.max_cache_size, change.new_config.max_cache_size);
}
...
async fn test_invalid_toml_config() {
    ...
    let initial_config = DnsConfig::default();
    let reloader = ConfigReloader::new(initial_config, Some(temp_file.path().to_string_lossy().to_string()));
...
async fn test_config_file_not_found() {
    let initial_config = DnsConfig::default();
    let reloader = ConfigReloader::new(initial_config, Some("/nonexistent/path/config.toml".to_string()));
