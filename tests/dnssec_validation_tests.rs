use heimdall::dnssec::{
    DnsSecAlgorithm, DigestType, DnsSecError, ValidationResult,
    calculate_key_tag, TrustAnchor, TrustAnchorStore, DnsSecValidator
};
...
fn test_digest_computation() {
    ...
    assert_eq!(hex::encode(&sha256_digest), "916f0027a575074ce72a331777c3478d6513f786a591bd892da1a577bf2335f9");
...
fn test_trust_anchor_store_hierarchy() {
    ...
    // Add anchor for .com
    let com_anchor = TrustAnchor::new(
        "com".to_string(),
        257,
        3,
        8,
...
fn test_dnssec_error_display() {
    let errors = vec![
        (DnsSecError::NoDnsKey, "No DNSKEY record found for validation"),
        (DnsSecError::NoDs, "No DS record found at parent zone"),
        (DnsSecError::NoRrsig, "No RRSIG record found for RRset"),
        (DnsSecError::SignatureExpired, "DNSSEC signature has expired"),
        (DnsSecError::SignatureNotYetValid, "DNSSEC signature is not yet valid"),
        (DnsSecError::KeyTagMismatch, "Key tag does not match"),
        (DnsSecError::UnsupportedAlgorithm(99), "Unsupported DNSSEC algorithm: 99"),
        (DnsSecError::UnsupportedDigestType(99), "Unsupported digest type: 99"),
        (DnsSecError::SignatureVerificationFailed, "DNSSEC signature verification failed"),
        (DnsSecError::DsDigestMismatch, "DS record digest does not match DNSKEY"),
        (DnsSecError::InvalidPublicKey, "Invalid DNSKEY public key format"),
        (DnsSecError::InvalidSignature, "Invalid RRSIG signature format"),
    ];
...
async fn test_validation_result_types() {
    ...
    assert_eq!(ValidationResult::Indeterminate, ValidationResult::Indeterminate);
}
