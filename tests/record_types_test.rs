use heimdall::dns::{DNSPacket, header::DNSHeader, question::DNSQuestion, enums::{DNSResourceType, DNSResourceClass}};

fn create_query(domain: &str, qtype: DNSResourceType) -> DNSPacket {
    let labels: Vec<String> = domain.split('.').map(|s| s.to_string()).chain(std::iter::once("".to_string())).collect();
...
fn test_all_dns_record_types_serialization() {
    ...
    for (domain, record_type) in test_cases {
        ...
        assert!(serialized.is_ok(), "Failed to serialize {:?} query for {}", record_type, domain);
        
        // Test that the serialized query can be parsed back
        let parsed = DNSPacket::parse(&serialized.unwrap());
        assert!(parsed.is_ok(), "Failed to parse back {:?} query for {}", record_type, domain);
...
fn test_dns_record_type_conversions() {
    ...
    for (number, expected_type) in test_mappings {
        ...
        assert_eq!(parsed_type, expected_type, "Failed u16->DNSResourceType conversion for {}", number);
        
        // Test Into<u16> conversion
        let back_to_number: u16 = expected_type.into();
        assert_eq!(back_to_number, number, "Failed DNSResourceType->u16 conversion for {:?}", expected_type);
    }
}
...
fn test_packet_validation_with_different_record_types() {
    for record_type in [
        ...
        assert!(query.valid(), "Query with {:?} record type should be valid", record_type);
    }
}
...
fn test_complex_domain_names() {
    ...
    for domain in complex_domains {
        ...
        let original_domain = query.questions[0].labels[..query.questions[0].labels.len()-1].join(".");
        let parsed_domain = parsed.questions[0].labels[..parsed.questions[0].labels.len()-1].join(".");
        assert_eq!(original_domain, parsed_domain, "Domain name should be preserved through serialization");
    }
}