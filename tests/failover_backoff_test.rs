...
async fn test_exponential_backoff_for_failed_servers() {
    ...
    for i in 0..5 {
        ...
        let question = DNSQuestion {
            labels: vec![format!("backoff{}", i), "test".to_string(), "com".to_string()],
        ...
        println!("Query {}: result={:?}, time={:?}", i, result.is_ok(), query_time);
    ...
    println!("Exponential backoff test - server health:\n{}", resolver.get_health_debug_info());
    
    let unreachable_server: SocketAddr = "192.0.2.1:53".parse().unwrap();
    
    if let Some(stats) = health_stats.get(&unreachable_server) {
        println!("Final stats: requests={}, failures={}, healthy={}", 
                stats.total_requests, stats.consecutive_failures, stats.is_healthy);
        
        assert!(stats.total_requests >= 3, "Should have tried the server multiple times");
        assert!(stats.consecutive_failures >= 3, "Should have multiple consecutive failures");
        assert!(!stats.is_healthy, "Server should be marked unhealthy after 3+ failures");
    }
}
...
async fn test_successful_failover_to_backup_servers() {
    ...
    let question = DNSQuestion {
        labels: vec!["failover".to_string(), "test".to_string(), "com".to_string()],
        qtype: DNSResourceType::A,
        qclass: DNSResourceClass::IN,
    };
    query.questions.push(question);
    ...
    assert!(result.is_ok(), "Query should succeed via failover to working server");
    ...
    let health_stats = resolver.get_server_health_stats();
    println!("Failover test - server health:\n{}", resolver.get_health_debug_info());
    ...
    if let Some(stats) = health_stats.get(&primary) {
        ...
        assert!(stats.consecutive_failures > 0, "Primary should have failures");
        println!("Primary: requests={}, failures={}", stats.total_requests, stats.consecutive_failures);
    }
    ...
    if let Some(stats) = health_stats.get(&secondary) {
        ...
        assert!(stats.consecutive_failures > 0, "Secondary should have failures");
        println!("Secondary: requests={}, failures={}", stats.total_requests, stats.consecutive_failures);
    }
    ...
    if let Some(stats) = health_stats.get(&tertiary) {
        ...
        assert!(stats.successful_responses > 0, "Tertiary should have succeeded");
        assert!(stats.is_healthy, "Tertiary should be healthy");
        println!("Tertiary: requests={}, successes={}", stats.total_requests, stats.successful_responses);
    }
    
    // Quaternary should not have been tried (tertiary succeeded first)
    if let Some(stats) = health_stats.get(&quaternary) {
        println!("Quaternary: requests={}, successes={}", stats.total_requests, stats.successful_responses);
...
async fn test_health_recovery_after_reset() {
    ...
    let question = DNSQuestion {
        labels: vec!["recovery".to_string(), "test".to_string(), "com".to_string()],
        qtype: DNSResourceType::A,
        qclass: DNSResourceClass::IN,
    };
    query.questions.push(question);
    ...
    if let Some(server_stats) = stats_before.get(&server_addr) {
        ...
        println!("Before: requests={}, successes={}, healthy={}", 
    ...
    if let Some(server_stats) = stats_after_reset.get(&server_addr) {
        ...
        println!("After reset: requests={}, failures={}, healthy={}", 
