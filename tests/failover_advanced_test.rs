...
async fn test_consecutive_failures_mark_unhealthy() {
    ...
    for i in 0..4 {
        ...
        let question = DNSQuestion {
            labels: vec![format!("test{}", i), "example".to_string(), "com".to_string()],
    ...
    println!("Final server health debug info:\n{}", resolver.get_health_debug_info());
    ...
    if let Some(stats_unreachable) = health_stats.get(&unreachable_server) {
        println!("Unreachable server stats: requests={}, failures={}, healthy={}", 
                stats_unreachable.total_requests, stats_unreachable.consecutive_failures, stats_unreachable.is_healthy);
        ...
        assert!(stats_unreachable.total_requests >= 1, "Unreachable server should have been tried at least once");
        assert!(stats_unreachable.consecutive_failures >= 1, "Should have at least 1 failure");
        ...
        println!("Note: Server appears healthy because health-based ordering prevents repeated failures");
    }
    
    if let Some(stats_working) = health_stats.get(&working_server) {
        println!("Working server stats: requests={}, successes={}, healthy={}", 
                stats_working.total_requests, stats_working.successful_responses, stats_working.is_healthy);
        assert!(stats_working.total_requests >= 4, "Working server should have handled the queries");
        assert_eq!(stats_working.successful_responses, stats_working.total_requests, "All requests to working server should succeed");
        assert!(stats_working.is_healthy, "Working server should remain healthy");
        assert!(stats_working.avg_response_time.is_some(), "Should have response time data");
    }
}
...
async fn test_health_based_priority_ordering() {
    ...
    for i in 0..6 {
        ...
        let question = DNSQuestion {
            labels: vec![format!("priority{}", i), "test".to_string(), "com".to_string()],
            qtype: DNSResourceType::A,
            qclass: DNSResourceClass::IN,
        };
        query.questions.push(question);
    ...
    let health_stats = resolver.get_server_health_stats();
    println!("Priority ordering test - server health:\n{}", resolver.get_health_debug_info());
    ...
    if let Some(stats) = health_stats.get(&unreachable) {
        ...
        assert!(stats.consecutive_failures >= 3, "Should have multiple failures");
    }
    ...
    if let Some(stats) = health_stats.get(&cloudflare) {
        ...
        assert!(stats.avg_response_time.is_some(), "Should have response time data");
    }
...
async fn test_server_health_reset() {
    ...
    if let Some(stats) = stats_before.get(&test_server) {
        assert!(stats.consecutive_failures >= 3, "Should have multiple failures");
        println!("Before reset: failures={}, healthy={}", stats.consecutive_failures, stats.is_healthy);
    }
    ...
    if let Some(stats) = stats_after.get(&test_server) {
        assert_eq!(stats.consecutive_failures, 0, "Failures should be reset to 0");
        assert!(stats.is_healthy, "Server should be marked healthy after reset");
        println!("After reset: failures={}, healthy={}", stats.consecutive_failures, stats.is_healthy);
    }
