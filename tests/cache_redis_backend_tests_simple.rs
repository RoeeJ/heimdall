...
async fn test_cache_backend_is_empty_default() {
    ...
    // Add entry
    let key = CacheKey::new("example.com".to_string(), DNSResourceType::A, DNSResourceClass::IN);
    let entry = create_test_entry(300);
...
async fn test_layered_cache_l1_only() {
    ...
    // Set entry
    cache.set(&key, packet.clone(), Duration::from_secs(300)).await;
    ...
    // Cache miss
    let miss_key = CacheKey::new("missing.com".to_string(), DNSResourceType::A, DNSResourceClass::IN);
...
async fn test_layered_cache_operations() {
    ...
    for i in 0..5 {
        let key = CacheKey::new(format!("example{}.com", i), DNSResourceType::A, DNSResourceClass::IN);
        let packet = DNSPacket::default();
...
async fn test_layered_cache_len() {
    ...
    for i in 0..3 {
        let key = CacheKey::new(format!("l1-{}.com", i), DNSResourceType::A, DNSResourceClass::IN);
        let entry = create_test_entry(300);
        l1.set(&key, entry).await;
    }
    
    for i in 0..2 {
        let key = CacheKey::new(format!("l2-{}.com", i), DNSResourceType::A, DNSResourceClass::IN);
        let entry = create_test_entry(300);
